...
public class CommandFramework implements CommandExecutor {
    ...
    private FrozedTeams plugin;

    public CommandFramework(FrozedTeams plugin) {
        this.plugin = plugin;
    ...
    public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (command.isAdminOnly() && !sender.hasPermission("frozedteams.admin")) {
                    sender.sendMessage(ChatColor.RED + "Only ops may execute this command.");
                    return true;
                }
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(ColorText.translate("&cIt seems you don't have enough perms to perform this command."));
                    return true;
                }
